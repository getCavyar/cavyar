...
import { shortenPublicKey, signInWithGithub } from "~/ts/utils";
  ...
  async (connected) => {
    if (
      (wallet.publicKey.value &&
        authData.value &&
        wallet.publicKey.value?.toString() !== authData.value?.user?.name) ||
      (!authData.value && wallet.publicKey.value)
    ) {
      await handleSignIn();
    } else if (connected && status.value === "unauthenticated") {
      await handleSignIn();
    } else if (!connected && status.value === "authenticated") {
      await signOut();
    }
  },
);
...
// when the user signs in with wallet, prompt the user to sign in with github
watch(
  () => authData.value?.user?.name,
  async (name) => {
    const userData: {
      data: User | null;
      statusCode: number;
      ...
      `/api/users/${wallet.publicKey.value!.toString()}/profile`,
      {
        method: "GET",
      },
    );
    ...
    if (userData.statusCode !== 200) {
      openDialog(DialogType.Error, "Error fetching user data");
    }

    if (name && !userData.data?.username) {
      openDialog(
        DialogType.Info,
        "Please sign in with Github to gain access to autogenerated snippet explanations",
        [
          {
            label: "Cancel",
            callback: () => {},
